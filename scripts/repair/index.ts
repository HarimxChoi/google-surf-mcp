...
const REPORT_PATH = resolve(dirname(fileURLToPath(import.meta.url)), 'repair-report.json');

interface RunOutput {
  trigger: 'none' | 'detected' | 'blocked';
  fault?: string;
  probe?: {
    resultsLen: number;
    h3Count: number;
...
async function detectAndRepair(): Promise<RunOutput> {
  ...
  let blocked = false;
  let brokenHtml = '';
  let brokenSignals: RunOutput['probe'] | null = null;
  ...
  try {
    ...
    for (const peer of STRATEGIES.slice(1)) {
      const out = await page.evaluate(parseResultsInBrowser, { strategy: peer, max: 10 });
      peerCounts.push(out.results.length);
    }
    ...
    const signals = {
      ...
      url: page.url(),
      peerResults: peerCounts.length ? Math.max(...peerCounts) : undefined,
    };
    const fault = classifyFault({ ...signals, responseStatus: 200, responseTimeMs: 1000 });

    if (fault.type === 'blocked') {
      blocked = true;
      brokenSignals = signals;
      console.error(`[repair] BLOCKED by Google, drift undetectable this run (${describeFault(fault)})`);
    } else if (fault.type === 'selector_broken') {
      triggerFault = true;
  ...
  if (blocked) {
    return { trigger: 'blocked', fault: 'blocked', probe: brokenSignals ?? undefined };
  }
  if (!triggerFault || !brokenSignals) {
    return { trigger: 'none', probe: brokenSignals ?? undefined };
  }
...
function writeReport(result: RunOutput | { error: string }): void {
  try {
    writeFileSync(REPORT_PATH, JSON.stringify(result, null, 2));
  } catch (e) {
    console.error('[repair] could not write report:', (e as Error).message);
  }
}
...
detectAndRepair()
  ...
  .catch((e) => {
    ...
    writeReport({ error: (e as Error).message });
    process.exit(1);
  });
